...
import {openLink, getPhotoSize} from './common';
...
function isMetaBubble(chatId, messageId) {
...
function isMessageUnread(chatId, messageId) {
  ...
  return is_outgoing
    ? id > last_read_outbox_message_id
...
function getMedia(
  ...
  switch (content['@type']) {
    ...
    default:
      ...
      return (
        <Text style={{color: 'red'}}>Not Supported: {content['@type']}</Text>
      );
  }
}
...
function showMessageForward(message): boolean {
  if (!message) {
    return false;
  }
...
function getFormattedText(formattedText) {
  if (formattedText['@type'] !== 'formattedText') {
    return null;
  }

  const {text, entities} = formattedText;
  // TODO: fix formattedText style
  if (!text) {
    return null;
  }
  if (!entities) {
  ...
  for (let i = 0; i < entities.length; i++) {
    ...
    if (textBefore) {
      ...
      if (textBefore) {
        result.push(<Text>{textBefore}</Text>);
      }
    }
    ...
    switch (type['@type']) {
      case 'textEntityTypeBold': {
        result.push(
          <Text key={entityKey} style={Base.boldText}>
            {entityText}
          </Text>,
        );
        break;
      ...
      case 'textEntityTypeBotCommand': {
        const command =
          entityText.length > 0 && entityText[0] === '/'
            ...
            : entityText;
        result.push(<Text key={entityKey}>{entityText}</Text>);
        break;
      }
      ...
      case 'textEntityTypeEmailAddress': {
        result.push(
          <Text
            key={entityKey}
            onPress={() => openLink(`mailto:${entityText}`)}>
            {entityText}
          </Text>,
        );
        break;
      ...
      case 'textEntityTypeHashtag': {
        result.push(
          <Text key={entityKey} style={Base.hashTagText}>
            {entityText}
          </Text>,
        );
        break;
      ...
      case 'textEntityTypeItalic': {
        result.push(
          <Text key={entityKey} style={Base.italicText}>
            {entityText}
          </Text>,
        );
        break;
      ...
      case 'textEntityTypePhoneNumber': {
        result.push(
          <Text key={entityKey} onPress={() => openLink(`tel:${entityText}`)}>
            {entityText}
          </Text>,
        );
        break;
      ...
      default:
        result.push(<Text>{entityText}</Text>);
        break;
    }
  ...
  if (index < text.length) {
    ...
    if (textAfter) {
      result.push(<Text>{textAfter}</Text>);
    }
  }
  // TODO: fix message element style
  return result;
}
...
function canMessageBeEdited(chatId, messageId) {
  const message = MessageStore.get(chatId, messageId);
  if (!message) {
    return false;
  }
...
function isMessageMuted(message) {
...
function getReplyMinithumbnail(chatId, messageId) {
  const message = MessageStore.get(chatId, messageId);
  if (!message) {
    return;
  }
...
function getEmojiMatches(chatId, messageId) {
  ...
  if (textContent['@type'] !== 'formattedText') {
    return 0;
  }
...
function openMedia(chatId, messageId, fileCancel = true) {
  ...
  switch (content['@type']) {
    ...
    // }
    case 'messagePhoto': {
      NavigationService.NavigationAction('navigate', 'ThumbViewer', {
        id: messageId,
      });
      // const { photo } = content;
      // if (photo) {
...
function checkInclusion(index, entities) {
  ...
  for (let i = 0; i < entities.length; i++) {
    if (
      index >= entities[i].offset &&
      index < entities[i].offset + entities[i].length
    ) {
      return true;
    }
  }
...
function checkIntersection(startIndex, endIndex, entities) {
  ...
  for (let i = 0; i < entities.length; i++) {
    if (
      startIndex <= entities[i].offset &&
      entities[i].offset + entities[i].length - 1 <= endIndex
    ) {
      return true;
    }
  }
...
function getEntities(text) {
  ...
  let childNodes = Array.from(result.childNodes);
  ...
  for (let c = 0; c < 2; c++) {
    ...
    while ((index = text.indexOf(checkString, lastIndex)) !== -1) {
      if (start === -1) {
        ...
        if (
          checkInclusion(index, entities) ||
          checkIntersection(start, index, entities)
        ) {
          start = -1;
          continue;
        }

...
function getServiceMessageContent(message, openUser = false) {
  ...
  const isChannel =
    chat.type['@type'] === 'chatTypeSupergroup' && chat.type.is_channel;
  ...
  switch (content['@type']) {
    ...
    case 'messageGameScore': {
      const messageGame = MessageStore.get(
        message.chat_id,
        content.game_message_id,
      );
      if (
        messageGame &&
        messageGame.content &&
    ...
    case 'messagePaymentSuccessful': {
      ...
      const messageInvoice = MessageStore.get(
        message.chat_id,
        content.invoice_message_id,
      );
      if (
        messageInvoice &&
    ...
    case 'messagePinMessage': {
      ...
      const pinnedMessage = MessageStore.get(
        message.chat_id,
        content.message_id,
      );

...
import {APP_NAME, NOTIFICATION_AUDIO_DELAY_MS} from '../constants/Constants';
        ...
        if (!windowFocused) {
          ...
          if (chatMap) {
            const newChatMap = new Map(
              [...chatMap].filter(
                ([id, m]) => m.id > last_read_inbox_message_id,
              ),
            );
        ...
        if (!windowFocused) {
          ...
          if (is_permanent && message_ids.length > 0) {
            ...
            if (chatMap) {
              ...
              const newChatMap = new Map(
                [...chatMap].filter(([id, m]) => !filterMap.has(id)),
              );
              if (newChatMap.size < chatMap.size) {
                this.newMessages.set(chat_id, newChatMap);
                this.updateTimer();
        ...
        if (!windowFocused) {
          ...
          if (
            ...
            if (now > this.nextSoundAt) {
              ...
              nextSoundAt.setMilliseconds(
                nextSoundAt.getMilliseconds() + NOTIFICATION_AUDIO_DELAY_MS,
              );
